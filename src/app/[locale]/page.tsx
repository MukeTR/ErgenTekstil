...
import { getContent, getProducts, productImageUrl } from "@/lib/data";
      ...
      <section className="bg-brand-grey-light py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            {c.collections.map((col: string) => (
              <div
                key={col}
                className="rounded-2xl bg-white px-4 py-8 text-center font-heading text-sm font-semibold text-brand-navy shadow-sm"
              >
                {col}
              </div>
            ))}
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            ...
            <p className="mt-6 text-brand-grey leading-relaxed">{c.processText}</p>
            <ul className="mt-8 grid grid-cols-2 gap-3">
              {c.processList.map((step: string) => (
                <li
                  key={step}
                  className="rounded-full bg-brand-grey-light px-4 py-2 text-center font-heading text-xs font-semibold text-brand-navy"
                >
                  {step}
                </li>
              ))}
